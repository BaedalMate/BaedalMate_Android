...
class PostCategoryAsiaFragment : Fragment() {
    ...
    private fun setCategoryClickListener() {
        ...
        binding.radiogroupLayoutPostCategoryAsiaSort.setOnCheckedChangeListener { group, checkedId ->
            when (checkedId) {
                R.id.radiobutton_post_category_asia_sort_time -> { getRecruitList(sort = "") }
                R.id.radiobutton_post_category_asia_sort_star -> { getRecruitList(sort = "") }
                R.id.radiobutton_post_category_asia_sort_popular -> { getRecruitList(sort = "") }
            }
        }
    }
}