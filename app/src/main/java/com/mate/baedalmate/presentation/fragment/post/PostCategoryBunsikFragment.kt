...
class PostCategoryBunsikFragment : Fragment() {
    ...
    private fun setCategoryListContents() {
        ...
        recruitViewModel.recruitListBunsik.observe(viewLifecycleOwner) { recruitList ->
            if (recruitList.recruitList.isNotEmpty()) {
                postCategoryListAdapter.submitList(recruitList.recruitList.toMutableList())
                with(constraintSet) {
                    clone(binding.layoutPostCategoryListBunsik)
                    setVisibility(emptyView.id, View.GONE)
                    applyTo(binding.layoutPostCategoryListBunsik)
                }
            } else {
                with(constraintSet) {
                    clone(binding.layoutPostCategoryListBunsik)
