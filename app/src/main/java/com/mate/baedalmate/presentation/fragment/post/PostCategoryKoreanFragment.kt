...
class PostCategoryKoreanFragment : Fragment() {
    ...
    private fun initListAdapter() {
        ...
        with(binding) {
            rvPostCategoryKoreanList.adapter = postCategoryListAdapter
        }
    }

    private fun setCategoryClickListener() {
        binding.radiogroupLayoutPostCategoryKoreanSort.setOnCheckedChangeListener { group, checkedId ->
            when (checkedId) {
                R.id.radiobutton_post_category_korean_sort_time -> {
                    getRecruitList(sort = "deadlineDate")
                }
                R.id.radiobutton_post_category_korean_sort_star -> {
                    getRecruitList(sort = "score")
                }
                R.id.radiobutton_post_category_korean_sort_popular -> {
    ...
    private fun setCategoryListContents() {
        ...
        recruitViewModel.recruitListKorean.observe(viewLifecycleOwner) { recruitList ->
            if (recruitList.recruitList.isNotEmpty()) {
                postCategoryListAdapter.submitList(recruitList.recruitList.toMutableList())
                with(constraintSet) {
                    clone(binding.layoutPostCategoryListKorean)
                    setVisibility(emptyView.id, View.GONE)
                    applyTo(binding.layoutPostCategoryListKorean)
                }
            } else {
                with(constraintSet) {
                    clone(binding.layoutPostCategoryListKorean)
