...
class PostCategoryKoreanFragment : Fragment() {
    ...
    private fun setCategoryClickListener() {
        binding.radiogroupLayoutPostCategoryKoreanSort.setOnCheckedChangeListener { group, checkedId ->
            when (checkedId) {
                R.id.radiobutton_post_category_korean_sort_time -> { getRecruitList(sort = "deadlineDate") }
                R.id.radiobutton_post_category_korean_sort_star -> { getRecruitList(sort = "score") }
                R.id.radiobutton_post_category_korean_sort_popular -> { getRecruitList(sort = "view") }
            }
        }
    }
}