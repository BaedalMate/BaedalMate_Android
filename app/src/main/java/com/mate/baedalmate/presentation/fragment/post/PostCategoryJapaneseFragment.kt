...
class PostCategoryJapaneseFragment : Fragment() {
    ...
    private fun initListAdapter() {
        ...
        with(binding) {
            rvPostCategoryJapaneseList.adapter = postCategoryListAdapter
        }
    }

    private fun setCategoryClickListener() {
        binding.radiogroupLayoutPostCategoryJapaneseSort.setOnCheckedChangeListener { group, checkedId ->
            when (checkedId) {
                R.id.radiobutton_post_category_japanese_sort_time -> {
                    getRecruitList(sort = "deadlineDate")
                }
                R.id.radiobutton_post_category_japanese_sort_star -> {
                    getRecruitList(sort = "score")
                }
                R.id.radiobutton_post_category_japanese_sort_popular -> {
    ...
    private fun setCategoryListContents() {
        ...
        recruitViewModel.recruitListJapanese.observe(viewLifecycleOwner) { recruitList ->
            if (recruitList.recruitList.isNotEmpty()) {
                postCategoryListAdapter.submitList(recruitList.recruitList.toMutableList())
                with(constraintSet) {
                    clone(binding.layoutPostCategoryListJapanese)
                    setVisibility(emptyView.id, View.GONE)
                    applyTo(binding.layoutPostCategoryListJapanese)
                }
            } else {
                with(constraintSet) {
                    clone(binding.layoutPostCategoryListJapanese)
