...
class PostCategoryJapaneseFragment : Fragment() {
    ...
    private fun setCategoryClickListener() {
        binding.radiogroupLayoutPostCategoryJapaneseSort.setOnCheckedChangeListener { group, checkedId ->
            when (checkedId) {
                R.id.radiobutton_post_category_japanese_sort_time -> { getRecruitList(sort = "deadlineDate") }
                R.id.radiobutton_post_category_japanese_sort_star -> { getRecruitList(sort = "score") }
                R.id.radiobutton_post_category_japanese_sort_popular -> { getRecruitList(sort = "view") }
            }
        }
    }
}