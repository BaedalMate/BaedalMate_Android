...
class PostCategoryPizzaFragment : Fragment() {
    ...
    private fun setCategoryClickListener() {
        ...
        binding.radiogroupLayoutPostCategoryPizzaSort.setOnCheckedChangeListener { group, checkedId ->
            when (checkedId) {
                R.id.radiobutton_post_category_pizza_sort_time -> { getRecruitList(sort = "") }
                R.id.radiobutton_post_category_pizza_sort_star -> { getRecruitList(sort = "") }
                R.id.radiobutton_post_category_pizza_sort_popular -> { getRecruitList(sort = "") }
            }
        }
    }
}